...
for sheetname, df in dfs.items():
    ...
    widedf = (
        df.assign(group=df.index // 3, pos=df.index % 3)
        .pivot(index="group", columns="pos", values=0)
        .rename(columns={0: "IndicatorName", 1: "col1", 2: "col2"})
    ...
    longdf = (
        widedf.melt(id_vars="Results", var_name="Date", value_name="IndicatorValue")
        .rename(columns={"Results": "IndicatorName"})
    )
    ...
    longdf["IndicatorValue"] = (
        longdf["IndicatorValue"]
        .astype(str)
        .str.replace(r"\s+", "", regex=True)
    )
    longdf["IndicatorValue"] = pd.to_numeric(
        longdf["IndicatorValue"], errors="coerce"
    ).fillna(0)
